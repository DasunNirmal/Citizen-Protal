...
async function sendChat(){
    ...
    fetch("/api/engagement", {
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body: JSON.stringify({user_id: profile_id, question_clicked: text,
        service: null})
    });
}
...
// Autosuggest for search input and chat input
let suggestTimer = null;
async function autosuggest(q){
    clearTimeout(suggestTimer);
    if(!q || q.length < 2){ document.getElementById("suggestions").innerHTML =
    ""; return; }
suggestTimer = setTimeout(async ()=>{
    const res = await
fetch(`/api/search/autosuggest?q=${encodeURIComponent(q)}`);
    const items = await res.json();
    const el = document.getElementById("suggestions");
    el.innerHTML = items.map(it=>`<div class="s-item"
onclick='pickSuggestion(${JSON.stringify(JSON.stringify(it))})'>${it.name?.en
|| it.name}</div>`).join("");
    }, 250);
}
...
async function profileSubmit(){
    const data1 = {name: document.getElementById("p_name").value, age:
document.getElementById("p_age").value};
    const data2 = {email: document.getElementById("p_email").value, phone:
document.getElementById("p_phone").value};
    const data3 = {job: document.getElementById("p_job").value};
    // send steps sequentially
    let res = await fetch("/api/profile/step", {method:"POST",
headers:{"Content-Type":"application/json"}, body:
JSON.stringify({email:data2.email, step:"basic", data:data1})});
    let j = await res.json();
    profile_id = j.profile_id || null;
    await fetch("/api/profile/step", {method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({profile_id: profile_id, step:"contact", data:data2})});
    await fetch("/api/profile/step", {method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({profile_id: profile_id, step:"employment", data:data3})});
...
async function loadAds(){
    ...
    el.innerHTML = ads.map(a=>`<div class="ad-card"><a href="${a.link ||
'#'}"><h4>${a.title}</h4><p>${a.body || ''}</p></a></div>`).join("");
}
